...
class PaymentDepositSagaIntegrationTest {
    ...
    void rollbackDeductionAndCompensatePaymentWhenDeductionFailsMidway() {
        ...
        Payment compensatedPayment = waitForPaymentStatus(savedPayment.getUuid(), PaymentStatus.FAILED, Duration.ofSeconds(3));
        ...
        assertThat(paymentSupport.hasHistoryType(compensatedPayment.getId(), PaymentHistoryType.PAYMENT_FAILED)).isTrue();
        verify(tossPaymentClient).cancel(eq("saga-test-payment-key"), argThat(body ->
                ((Number) body.get("cancelAmount")).longValue() == 5000L));
    }
    ...
    void completeDeductionWithoutCompensationWhenBalanceIsEnough() {
        ...
        assertThat(userHistories.stream().map(DepositHistory::getType))
                .containsExactlyInAnyOrder(DepositHistoryType.CHARGE, DepositHistoryType.USE, DepositHistoryType.USE);
        verify(increaseDepositUseCase).increase(
                eq(SystemDepositInitData.SYSTEM_USER_UUID),
                eq(8000L),
                eq(DepositHistoryType.DEPOSIT_CHARGE),
                eq(orderUuid));
    ...
    void markRollbackFailedWhenCompensationCancelFails() {
        ...
        assertThat(paymentSupport.hasHistoryType(rollbackFailedPayment.getId(), PaymentHistoryType.PAYMENT_ROLLBACK_FAILED))
                .isTrue();
        verify(tossPaymentClient).cancel(eq("saga-compensation-fail-payment-key"), argThat(body ->
                ((Number) body.get("cancelAmount")).longValue() == 5000L));
    }
    ...
    void compensateWithoutAnyDeductionWhenDepositIsInsufficientFromStart() {
        ...
        assertThat(compensatedPayment.getPaymentStatus()).isEqualTo(PaymentStatus.FAILED);
        assertThat(paymentSupport.hasHistoryType(compensatedPayment.getId(), PaymentHistoryType.PAYMENT_FAILED)).isTrue();
        verify(tossPaymentClient).cancel(eq("saga-insufficient-payment-key"), argThat(body ->
                ((Number) body.get("cancelAmount")).longValue() == 5000L));
    }
    ...
    void completeEventChainWhenPaymentSuccessEventAndDepositDeductionSuccess() {
        ...
        assertThat(paymentSupport.hasHistoryType(latestPayment.getId(), PaymentHistoryType.PAYMENT_FAILED)).isFalse();
        ...
        verify(increaseDepositUseCase, atLeast(1)).increase(
                eq(SystemDepositInitData.SYSTEM_USER_UUID),
                eq(5000L),
                eq(DepositHistoryType.PG_CHARGE),
                eq(order.getUuid()));
        verify(tossPaymentClient, never()).cancel(anyString(), anyMap());
    }
    ...
    void rollbackEventChainWhenPaymentSuccessEventAndDepositDeductionFail() {
        ...
        Payment failedPayment = waitForPaymentStatus(payment.getUuid(), PaymentStatus.FAILED, Duration.ofSeconds(3));
        ...
        assertThat(paymentSupport.hasHistoryType(failedPayment.getId(), PaymentHistoryType.PAYMENT_FAILED)).isTrue();
        ...
        assertThat(userHistories).hasSize(1);
        assertThat(userHistories.get(0).getType()).isEqualTo(DepositHistoryType.CHARGE);
        verify(tossPaymentClient).cancel(eq("event-chain-fail-payment-key"), argThat(body ->
                ((Number) body.get("cancelAmount")).longValue() == 5000L));
    }
