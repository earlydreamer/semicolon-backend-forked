...
> 주의: 이 문서는 EC2 운영 시절 기록이다. 현재 자동 배포 기준은 `.github/workflows/deploy-m1-tunnel.yml`과 `k8s/README.md`를 따른다.
...
- **AI 전용 워크플로우 생성**: 특정 기능(AI 등)만 빠른 패치가 가능하도록 전용 워크플로우를 분리해 운영했습니다.
...
- **적용**: 배포 워크플로우에서 `JAVA_TOOL_OPTIONS` 환경변수를 통해 런타임에 주입하도록 통일했습니다.
...
- **호스트 네임 표준화**: 로컬 중심의 설정을 탈피하고, K8s 서비스 명칭(`user-service`, `redis`, `elasticsearch`, `kafka`)을 기본값으로 사용하도록 release 프로파일을 전면 개편했습니다.
...
- **AI 서비스**:
  - `application-common.yml` 임포트 누락으로 인한 Redis 연결 실패(localhost 접속) 문제 해결.
  - `show_sql` 오타 수정.
- **User 서비스**:
...
1. **DB 스키마 정책**: 현재 초기 테이블 생성을 위해 Secret(`semicolon-env`)의 `SPRING_JPA_HIBERNATE_DDL_AUTO`를 `update`로 설정해 둔 경우가 있습니다. 모든 서비스가 안착하면 다시 `validate`로 수동 원복을 권장합니다.
