...
/**
 * 정산 수동 실패 처리 UseCase
 * - Settlement 상태: PROCESSING → FAILED
 * - 관리자가 정산을 수동으로 실패 처리
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class ManualFailSettlementUseCase {

    private final SettlementSupport settlementSupport;

    @Transactional
    public Settlement execute(UUID settlementUuid) {
        log.info("[정산 수동 실패 처리] settlementUuid={}", settlementUuid);

        Settlement settlement = settlementSupport.findByUuid(settlementUuid);
        settlement.fail();
        settlementSupport.save(settlement);

        log.info("[정산 수동 실패 처리됨] settlementUuid={}, status={}",
